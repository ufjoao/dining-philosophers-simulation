...
class ControllablePhilosopher(threading.Thread):
    def __init__(self, p_id, left_fork, right_fork, pause_event, **kwargs):
        ...
        self.fork_count = 0 # NOVO: Contador de garfos

        self.waiter = kwargs.get('waiter')
        self.barrier = kwargs.get('barrier')
        self.sleep_between_forks = kwargs.get('sleep_between_forks', 0)
        
        # Envia estado inicial para a GUI
        self._send_update()

    # ALTERADO: Método unificado para enviar todas as informações para a GUI
    ...
    def pickup_forks(self):
        ...
        if self.waiter: # Modo Garçom
            update_queue.put(('call_waiter', self.p_id)) # NOVO: Sinaliza chamada ao garçom
            self.waiter.acquire()
            update_queue.put(('end_call', self.p_id)) # NOVO: Sinaliza fim da chamada
            
            self.left_fork.acquire()
            self.fork_count = 1
            self._send_update() # Atualiza a GUI com 1 garfo
    ...
    def run(self):
        while self.running:
            ...
            if not self.running: break # Checagem extra para saída rápida

class App(tk.Tk):
    def __init__(self, *args, **kwargs):
        tk.Tk.__init__(self, *args, **kwargs)
        self.title("Jantar dos Filósofos - Simulação")
        self.geometry("700x600") # Aumentei a largura para a legenda
    ...
    def show_frame(self, page_name):
        ...
        if page_name == "SimulationFrame":
            # ALTERADO: Prepara a tela de simulação para o modo escolhido
            frame.prepare_for_simulation(self.simulation_type)
    ...
    def start_simulation(self, sim_type):
        ...
        for i in range(5):
            ...
            # Ordem dos garfos para a solução de hierarquia
...
class SimulationFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Frame da legenda e controles (à direita)
        right_frame = tk.Frame(main_frame)
        right_frame.pack(side="right", fill="y", padx=10, pady=20)
        
        # NOVO: Legenda dos estados
        legend_frame = tk.LabelFrame(right_frame, text="Legenda", font=tkfont.Font(family='Helvetica', size=10, weight="bold"))
        ...
        for state, color in states.items():
            ...
            tk.Label(f, text=" ", bg=color, relief="solid", borderwidth=1).pack(side="left", ipadx=5)
            tk.Label(f, text=f" = {state.title()}").pack(side="left", anchor="w")
        ...
        self.pause_btn.pack(fill="x", padx=5, pady=2)
    ...
    def prepare_for_simulation(self, sim_type):
        self.title_label.config(text=sim_type.replace("_", " ").title())
        self.canvas.delete("all") # Limpa o canvas para a nova simulação
        self.philosopher_shapes = {}
        self.fork_count_texts = {} # NOVO: Guarda os textos da contagem de garfos

        # Desenha garçom apenas se for a simulação correta
        if sim_type == "solucao_garcom":
            x, y = self.waiter_coords
            self.canvas.create_rectangle(x-15, y-15, x+15, y+15, fill="lightblue", outline="black", tags="waiter")
            self.canvas.create_text(x, y, text="Garçom")

        for i in range(5):
            x, y = self.philosopher_coords[i]
            shape = self.canvas.create_oval(x - 20, y - 20, x + 20, y + 20, fill="gray", tags=f"p_shape_{i}")
            label = self.canvas.create_text(x, y - 30, text=f"Filósofo {i}")
            fork_text = self.canvas.create_text(x, y, text="0", font=tkfont.Font(size=12, weight="bold")) # NOVO
            self.philosopher_shapes[i] = shape
            self.fork_count_texts[i] = fork_text
        ...
        try:
            while not update_queue.empty():
                ...
                if msg_type == 'status_update':
                    p_id, status, fork_count = message[1], message[2], message[3]
                    ...
                    self.canvas.itemconfig(self.fork_count_texts[p_id], text=str(fork_count)) # NOVO
                
                elif msg_type == 'call_waiter': # NOVO
                    p_id = message[1]
                    px, py = self.philosopher_coords[p_id]
                    wx, wy = self.waiter_coords
                    line = self.canvas.create_line(px, py, wx, wy, fill="blue", dash=(4, 2), tags=f"line_{p_id}")
                    self.waiter_lines[p_id] = line
                
                elif msg_type == 'end_call': # NOVO
                    p_id = message[1]
                    if p_id in self.waiter_lines:
                        self.canvas.delete(self.waiter_lines[p_id])
                        del self.waiter_lines[p_id]
        finally:
